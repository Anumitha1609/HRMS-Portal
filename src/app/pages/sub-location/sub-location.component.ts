...
export class SubLocationComponent implements OnInit, OnDestroy {
  ...
  // Form Group
  subLocationForm: FormGroup = this.fb.group({
  ...
  loadRecords() {
    ...
    this.masterService.getMasters('sublocation').subscribe({
      next: (data) => {
      ...
      error: (err) => {
        this.toastService.error('Failed to load sub-locations: ' + (err.error?.error || err.message));
        this.isLoading.set(false);
      }
    });
  }
  ...
  private subLocationFormPatchName(name: string) {
    this.subLocationForm.patchValue({ name }, { emitEvent: false });
  }
  ...
  onSave() {
    const { name, latitude, longitude, address } = this.subLocationForm.value;

    if (!name || !name.trim()) {
      this.subLocationForm.get('name')?.markAsTouched();
      this.toastService.warning('Sub-Location Name is mandatory.');
      return;
    }
  ...
  private saveRecord() {
    ...
    const { name, latitude, longitude, address, city, state, country, pinCode, timezone } = this.subLocationForm.value;
    const selected = this.selectedRecord();
    ...
    if (selected) {
      // Update
      this.masterService.updateMaster('sublocation', selected.id, name, extraData).subscribe({
        next: (updated) => {
          this.toastService.success('Sub-location updated successfully!');
        ...
        error: (err) => {
          this.toastService.error(err.error?.error || 'Failed to update sub-location');
        }
      });
    } else {
      // Create
      this.masterService.createMaster('sublocation', name, extraData).subscribe({
        next: (created) => {
          this.toastService.success('Sub-location created successfully!');
        ...
        error: (err) => {
          this.toastService.error(err.error?.error || 'Failed to create sub-location');
        }
      });
    }
  }
  ...
  onDelete(record: MasterRecord) {
    ...
    const confirmMessage = hasEmployees
      ? `Warning: "${record.name}" still has ${record.employeeCount} employee(s) assigned to it. Deleting it will leave those employee records pointing at a missing sub-location. Are you sure you want to continue?`
      : `Are you sure you want to soft-delete the sub-location "${record.name}"?`;

    if (confirm(confirmMessage)) {
      this.masterService.deleteMaster('sublocation', record.id).subscribe({
        next: () => {
          this.toastService.success('Sub-location deleted successfully!');
          this.selectedRecord.set(null);
          this.subLocationForm.reset();
          this.isEditing.set(false);
          this.subLocationForm.disable();
        ...
        error: (err) => {
          this.toastService.error(err.error?.error || 'Failed to delete sub-location');
        }
      });
    }
  }
